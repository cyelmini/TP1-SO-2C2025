...
int main (int argc, char *argv[]){
    ...
    printf("width: %d\nheight: %d\ndelay: %d\ntimeout: %d\nseed: %d\nview: %s\nnum_players: %d\n",
        width, height, delay, timeout, seed, view == NULL ? "-" : view, player_count);
    ...
    while(!game->gameFinished){
        ...
        int blocked = 0;
        for(unsigned int i = 0; i < player_count; i++){
            if(game->players[i].isBlocked) blocked++;
        }
        if(blocked == (int)player_count){
            game->gameFinished = true;
        }

        // Verificar timeout sin movimientos válidos
    ...
    for(unsigned int i = 0; i < player_count; i++){
        ...
        printf("Jugador %d terminó con código %d, puntaje %d / %d / %d\n",
            i, WEXITSTATUS(status), game->players[i].score, game->players[i].validMoves, game->players[i].invalidMoves);
